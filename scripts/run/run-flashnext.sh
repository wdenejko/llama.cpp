...
KV_USER="${KV:+1}"
UB_USER="${UB:+1}"
MD="${MD:-/home/wdenejko/models/Qwen3.8-Flash-Next-GGUF/MTP/mtp-Qwen3.8-Flash-Next-Q8_0-hcfix.gguf}"   # official Unsloth Q8 MTP head (hc tensors renamed to load; see rename_hc_tensors.py)
MDQ4=/home/wdenejko/models/Qwen3.8-Flash-Next-GGUF/MTP/mtp-Qwen3.8-Flash-Next-Q4_K_M-hcfix.gguf

# --- knobs ----------------------------------------------------------------------------
QUANT="${QUANT:-IQ4_XS}"             # default served model (Uncensored finetune); QUANT=Q4_K_XL for the Unsloth base
PORT="${PORT:-8080}"
NGL="${NGL:-99}"
...
# Deep-context guard (DEEPMTP=0 disables all of it). Thresholds are measured envelopes:
# Q8 draft host-OOMs past ~129k; KV=q8_0 frees ~3.9GB (a <1% wash); UB drops to 1024 past ~196k
# on the heavier Q4_K_XL (ub1024 completes a full 252k fill; ub2048 is OOM-killed at ~192k of
# ACTUAL fill / device-losts at load). IQ4_XS is lighter and keeps ub2048 at full depth.
if [ "${DEEPMTP:-1}" != "0" ]; then
  # Q4-draft downgrade at >98304 REMOVED 2026-09-02: the mmap-PLE fix freed ~28G host RAM, so the
  # higher-acceptance Q8 draft (default MD) now fits at full 262k (measured avail ~33G at boot).
  # Force the smaller Q4 draft manually with MD=$MDQ4 under extreme concurrency if ever needed.
  ...
  if [ -z "$UB_USER" ] && [ "$CTX" -gt 196608 ] && [ "$QUANT" != "IQ4_XS" ]; then
    UB=1024
    echo "[run-flashnext] deep-ctx: CTX=$CTX > 196608 on $QUANT — UB=1024 so a full-depth fill survives (ub2048 OOMs at ~192k fill; set UB=2048 to force, DEEPMTP=0 disables)" >&2
  fi
fi
PARALLEL="${PARALLEL:-1}"            # 1 = fastest per request
UB="${UB:-2048}"                     # physical batch; deep-ctx guard drops to 1024 past ~196k on Q4_K_XL
...
if [ -z "${MODEL:-}" ]; then
  case "$QUANT" in
    ...
    Q4_K_XL) MODEL="$HOME/models/Qwen3.8-Flash-Next-GGUF/UD-Q4_K_XL/Qwen3.8-Flash-Next-UD-Q4_K_XL-00001-of-00004.gguf" ;;
    *)       echo "[run-flashnext] unknown QUANT=$QUANT (set MODEL=<path>, or use IQ4_XS|Q4_K_XL)" >&2; exit 1 ;;
  esac
fi
if [ ! -f "$MODEL" ]; then
  echo "[run-flashnext] model not found ($QUANT): $MODEL" >&2
  exit 1
fi
...
# the Uncensored (IQ4_XS) template only accepts xhigh|medium|low and rejects 'high'
...
# the Uncensored (IQ4_XS) template rejects mid-conversation system messages (agent harnesses
# like opencode send them); serve a relaxed copy that renders them in place. TPLFIX=0 reverts.
# IQ4_XS only — the Q4_K_XL base keeps its own built-in template.
...
if [ "$MTP" = "1" ]; then
  UBD="${UBD:-512}"     # draft ubatch — decoupled so the TARGET keeps its own ub (1024)
...
echo "[run-flashnext] build=build-v2 quant=$QUANT ctx=$CTX ub=$UB ngl=$NGL fa=$FA port=$PORT ngram_offload=$NGRAM_OFFLOAD reasoning=$REASONING temp=$TEMP mtp=$MTP coopmat=$COOPMAT free_gpu=$FREE_GPU parallel=$PARALLEL" >&2
